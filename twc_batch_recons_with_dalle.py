...
def stack_reconstructions(input,x5, x1, titles=[]):
  #font = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-BoldItalic.ttf", 18)
  assert input.size == x1.size == x5.size
...
def reconstruction_pipeline(modelvqf4noattn,encoder_dalle,decoder_dalle,url,device, size=320,is_local=False):
    titles=["Input","VQVAE(DALLE)", "Reconstructed(VQF4)"] #no attn
...
def load_models(params):
    ...
    return model,encoder_dalle,decoder_dalle

def reconstruct(model,encoder_dalle,decoder_dalle,input_file,output_file,size):
    DEVICE = torch.device("cpu")
    # also disable grad to save memory
    torch.set_grad_enabled(False)

    img,inp,encoded,recons = reconstruction_pipeline(model,encoder_dalle,decoder_dalle, url=input_file,device=DEVICE, size=size,is_local=True)
...
def batch_gen(params):
    ...
    model,encoder_dalle,decoder_dalle = load_models(params)
    make_dir(output_dir)
        ...
        if  os.path.isfile(input_file):
            if (input_file.endswith(".jpeg") or input_file.endswith(".jpg") or input_file.endswith(".png")):
                ...
                if (file_already_exists(output_file)):
                    ...
                    continue;
                inp,encoded,recons = reconstruct(model,encoder_dalle,decoder_dalle,input_file,output_file,params.size)
                inp_vecs.append(inp)
