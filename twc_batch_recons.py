...
def reconstruction_pipeline(modelvqf4noattn,url,device, size=320,is_local=False):
    ...
    img = stack_reconstructions(custom_to_pil(preprocess_vqgan(x_vqgan[0])),
                              custom_to_pil(x1[0]), 
                              titles=titles)
    return img,inp.flatten().tolist(),encoded,x1.flatten().tolist()
...
def save_vectors(vecs,out_file,as_array):
    ...
    for file_names in os.listdir(input_dir):
        ...
        if  os.path.isfile(input_file):
            if (input_file.endswith(".jpeg") or input_file.endswith(".jpg") or input_file.endswith(".png")):
                ...
                inp_vecs.append(inp)
                if (first):
                    encoded_vecs = encoded.detach().cpu()
                    first = False
                else:
                    encoded_vecs = np.append(encoded_vecs,encoded.detach().cpu(),axis=0)
                recons_vecs.append(recons)
                out_str = f"{file_only}\n"
                log_fp.write(out_str)
    log_fp.close()
    assert(len(inp_vecs) == len(encoded_vecs) == len(recons_vecs))
    save_vectors(inp_vecs,output_dir + "/input_vectors.npy",True)
    save_vectors(encoded_vecs,output_dir + "/encoded_vectors.npy",False)
    save_vectors(recons_vecs,output_dir + "/recons_vectors.npy",True)
